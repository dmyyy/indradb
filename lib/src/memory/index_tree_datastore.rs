...
/// In-memory datastore backed by a Dynamic Order Statistic B-Tree
/// Optimal for read-heavy workloads at the expense of write performance - make sure to benchmark
/// for your use case.

/// Getting the i-th element is an O(1) operation - maintaining a cache of graph vertex/edge/property
/// indices allows for constant time access to specific graph components at the cost of some memory
/// overhead and time spent on cache invalidation.
///
/// The cache is configurable - pick and choose which parts you want to be cached. Should only be
/// enabled for surfaces that are written to rarely - invalidation happens on every write to the
/// underlying B-Tree.
#[derive(Debug, Default, Serialize, Deserialize)]
struct Cache {
    vertex_cache: Option<HashMap<Uuid, usize>>,
    edge_cache: Option<HashMap<Edge, usize>>,
    reversed_edge_cache: Option<HashMap<Edge, usize>>,
    vertex_property_cache: Option<HashMap<(Uuid, Identifier), usize>>,
    edge_property_cache: Option<HashMap<(Edge, Identifier), usize>>,
}

struct CacheConfig {
    enable_vertex_cache: bool,
    enable_edge_cache: bool,
    enable_reversed_edge_cache: bool,
    enable_vertex_property_cache: bool,
    enable_edge_property_cache: bool,
}

impl Cache {
    fn new(config: CacheConfig) -> Self {
        Self {
            vertex_cache: if config.enable_vertex_cache {
                Some(HashMap::new())
            } else {
                None
            },
            edge_cache: if config.enable_edge_cache {
                Some(HashMap::new())
            } else {
                None
            },
            reversed_edge_cache: if config.enable_reversed_edge_cache {
                Some(HashMap::new())
            } else {
                None
            },
            vertex_property_cache: if config.enable_vertex_property_cache {
                Some(HashMap::new())
            } else {
                None
            },
            edge_property_cache: if config.enable_edge_property_cache {
                Some(HashMap::new())
            } else {
                None
            },
...
/// Smaller node sizes give better write performance at the expense of
struct InternalMemoryConfig {
    vertices_node_size: usize,
    edges_node_size: usize,
    reversed_edges_node_size: usize,
    vertex_properties_node_size: usize,
    edge_properties_node_size: usize,
}

impl Default for InternalMemoryConfig {
    fn default() -> Self {
        Self {
            vertices_node_size: 1024,
            edges_node_size: 1024,
            reversed_edges_node_size: 1024,
            vertex_properties_node_size: 1024,
            edge_properties_node_size: 1024,
        }
    }
}

impl InternalMemory {
    fn new(config: InternalMemoryConfig, cache_config: CacheConfig) -> Self {
        Self {
            vertices: BTreeMap::with_maximum_node_size(config.vertices_node_size),
            edges: BTreeSet::with_maximum_node_size(config.edges_node_size),
            reversed_edges: BTreeSet::with_maximum_node_size(config.reversed_edges_node_size),
            vertex_properties: BTreeMap::with_maximum_node_size(config.vertex_properties_node_size),
            edge_properties: BTreeMap::with_maximum_node_size(config.edge_properties_node_size),
